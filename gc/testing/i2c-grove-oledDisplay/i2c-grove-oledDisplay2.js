...
　　     if(this.sequence != null){
   　      console.log("OledDisplay.playSequence(): error! (multiple call)");
    ...
    　   }
    　  var busy = 0;
        this.sequence = setInterval(async() => {
          if (busy != 0) {
             console.log("@@@@@@ playSequence() interval busy");
          } else {
            busy = 1;        
            for(var cnt=0;cnt < OLED_CONST.packetSize;cnt ++){
              console.log("@@@@@@ playSequence() loop%d",cnt);
              if(this.funcQueue[this.index].mode === OLED_CONST.commandMode){
                console.log("@@@@@@ playSequence() cmd");
                await this.i2cSlave.write8(OLED_CONST.commandMode,this.funcQueue[this.index].param);
              } else{
                console.log("@@@@@@ playSequence() data");
                await this.i2cSlave.write16(OLED_CONST.dataMode,this.funcQueue[this.index].param);
              }
              this.index ++;
